        ...
        <el-table v-else
          ...
          <el-table-column :label="$t('Config')" width="255">
            <template slot-scope="scope">
              ...
              <span>
                #{{ scope.row.func_extraConfigJSON.queue || $store.getters.SYSTEM_INFO('_FUNC_TASK_QUEUE_DEFAULT') }}
              </span>
            ...
            <template slot-scope="scope">
              <el-tooltip effect="dark" :content="scope.row.localFuncTaskRecordTip" placement="left" :disabled="!scope.row.localFuncTaskRecordTip">
                <el-badge class="task-record-button" type="primary" :max="99" :hidden="!funcTaskRecordCountMap[scope.row.id] || !scope.row.isLocalFuncTaskRecordAvailable" :value="funcTaskRecordCountMap[scope.row.id] && funcTaskRecordCountMap[scope.row.id].count || 0">
                  <el-link
                    @click="common.goToPage('task-record-func-list', { origin: 'syncAPI', originId: scope.row.id }, { hlDataId: scope.row.id })"
      ...
      switch(operation) {
        case 'disable':
          apiRes = await this.T.callAPI('post', '/api/v1/sync-apis/:id/do/modify', {
            ...
            feedback: { okMessage: this.$t('Sync API disabled') },
          });
          break;
        ...
        case 'enable':
          apiRes = await this.T.callAPI('post', '/api/v1/sync-apis/:id/do/modify', {
            ...
            feedback: { okMessage: this.$t('Sync API enabled') },
          });
          break;
        ...
        case 'delete':
          apiRes = await this.T.callAPI('/api/v1/sync-apis/:id/do/delete', {
            params  : { id: d.id },
            feedback: { okMessage: this.$t('Sync API deleted') },
          });
          break;
      }
      if (!apiRes || !apiRes.ok) return;
