...
MIN_UNIX_TIMESTAMP    = arrow.get('2000-01-01T00:00:00Z').int_timestamp
...
def get_memory_usage():
    '''
    Get the size of the memory usage
    '''
    pid = os.getpid()
...
def no_duplication(arr):
    dumps_arr = []
    for x in arr:
        x_dumps = json_dumps(x)
        if x_dumps not in dumps_arr:
            dumps_arr.append(x_dumps)

    return list(map(lambda x: json_loads(x), dumps_arr))
...
def get_timestamp_ms(d=None):
    timestamp_ms = int(get_arrow(d).float_timestamp * 1000)
    return timestamp_ms

def get_timestamp(d=None, ndigits=0):
    timestamp = get_arrow(d).float_timestamp

    if ndigits == 0:
        return int(timestamp)
    else:
        return round(timestamp, ndigits)
...
def is_past_datetime(d):
    ...
    return ts > get_timestamp(None, 3)

def get_days_from_now(d):
    ts = to_unix_timestamp(d)
    days = float(ts - get_timestamp(None, 3)) / 3600 / 24
    return days
...
def group_by_count(arr, count=1):
    '''
    Split the array into smaller arrays by `count`.
    '''
...
def gen_reg_exp_by_wildcard(pattern, simple_mode=False):
...
def match_wildcard(value, pattern, simple_mode=False):
    reg_exp = gen_reg_exp_by_wildcard(pattern, simple_mode=simple_mode)
...
def match_wildcards(value, patterns, simple_mode=False):
    ...
    for p in patterns:
        if match_wildcard(value, p, simple_mode=simple_mode):
            return True
...
def to_debug_text(text):
    return re.sub('\s+', ' ', text, flags=re.M).strip()

class TriggerLimit(object):
    _data = {}

    @classmethod
    def is_free(cls, key, interval):
        now  = get_timestamp()
        prev = cls._data.get(key) or 0
