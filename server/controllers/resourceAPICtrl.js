...
exports.list = function(req, res, next) {
  ...
  // Avoid accessing files out of resource folder
...
exports.operate = function(req, res, next) {
  ...
  // Check if operate target exists or not
  switch(operation) {
    case 'zip':
    case 'tar':
    case '7z':
    ...
    case 'rm':
      ...
      if (!fs.existsSync(targetAbsPath)) {
        return next(new E('EBizCondition', 'File or folder does not exist', { name: targetName }));
      }
      break;
    ...
    default:
      ...
      if (fs.existsSync(targetAbsPath)) {
        return next(new E('EBizCondition', 'File or folder already exists', { name: targetName }));
      }
      break;
  }
  ...
  switch(operation) {
    ...
    case 'zip':
      ...
      cmd = 'zip';
      cmdArgs.push('-q', '-r', zipFileName, targetName); // Execute in current folder
      break;
    ...
    case 'tar':
      ...
      cmd = 'tar';
      cmdArgs.push('-czf', tarFileName, targetName); // Execute in current folder
      break;
    ...
    case '7z':
      ...
      cmd = '7za';
      cmdArgs.push('a', p7zFileName, targetName); // Execute in current folder
      break;
