...
var MIN_UNIX_TIMESTAMP    = toolkit.MIN_UNIX_TIMESTAMP    = moment('2000-01-01T00:00:00Z').unix();
...
var sort = toolkit.sort = function sort(arr) {
  return arr.sort((a, b) => {
    if (a < b) return -1;
...
var noDuplication = toolkit.noDuplication = function noDuplication(arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    var xDumps = JSON.stringify(arr[i]);
    if (dumpsArr.indexOf(xDumps) < 0) {
      dumpsArr.push(xDumps);
    }
  }

  return dumpsArr.map(function(x) {
    return JSON.parse(x);
  });
};
 ...
 * @return {Integer}
 */
var getTimestampMs = toolkit.getTimestampMs = function getTimestampMs(d) {
  var timestampMs = getMoment(d).valueOf();
  return timestampMs;
};
 ...
 * @return {Integer}
 */
var getTimestamp = toolkit.getTimestamp = function getTimestamp(d, ndigits) {
  var timestamp = getTimestampMs(d) / 1000;
 ...
 * @return {String}
 */
var genRegExpByWildcard = toolkit.genRegExpByWildcard = function genRegExpByWildcard(pattern, simpleMode) {
 ...
 * @return {Boolean}
 */
var matchWildcard = toolkit.matchWildcard = function matchWildcard(value, pattern, simpleMode) {
  var regExp = toolkit.genRegExpByWildcard(pattern, simpleMode);
 ...
 * @return {Boolean}
 */
var matchWildcards = toolkit.matchWildcards = function matchWildcards(value, patterns, simpleMode) {
  if (!value || !patterns) {
    return false;
  }
  ...
  for (var i = 0;  i < patterns.length; i++) {
    ...
    if (toolkit.matchWildcard(value, p, simpleMode)) {
      return true;
    }
  }
...
var toDebugText = toolkit.toDebugText = function toDebugText(text) {
