...
router.all('*', function prepareBasicClientInfo(req, res, next) {
  ...
  // clientUILocale
  res.locals.clientUILocale = req.get(CONFIG._WEB_CLIENT_UILOCALE_HEADER) || 'en';
...
router.all('*', function prepareDBCacheUserAccess(req, res, next) {
  ...
  switch(CONFIG.DB_ENGINE) {
    case 'postgresql':
      dbHelper = require('./extraHelpers/postgresqlHelper');
      break;

    case 'mysql':
    default:
      dbHelper = require('./extraHelpers/mysqlHelper');
      break;
  }

  res.locals.db          = dbHelper.createHelper(reqLogger);
...
router.all('*', function prepareFunctionalComponents(req, res, next) {
  ...
  function _recordAbnormalReq(respBody) {
    ...
    if (respBody) {
      if (res.statusCode >= 400) {
        // Record entire response body if error occured
        reqInfo.respBody = respBody;
      } else {
        // Record limited response body dump if OK
  ...
  res.locals.getDateTimeRange = function getDateTimeRange(startField, endField) {
    ...
    if (start && end) {
      var range = toolkit.sort([start, end]);
      start = range[0];
      end   = range[1];
    }
  ...
  res.locals.sendData = function(ret) {
    // Return data, Detect `export` and `charset` option automatically.
    if (req.query.export) {
